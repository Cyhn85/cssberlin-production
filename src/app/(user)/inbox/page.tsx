...
function getOfferSummary(offer: OfferContext, currentUserId?: string | null) {
  ...
  if (latestOrder) {
    return {
      ...
      actionHref: currentUserId === offer.buyerId ? `/purchases/${latestOrder.id}` : `/sales/${latestOrder.id}`,
  ...
  if (offer.product.status === 'SOLD') {
    return {
      text: 'Der Artikel wurde bereits verkauft und kann nicht mehr abgeschlossen werden.',
      actionHref: '/offers',
  ...
  if (offer.product.status === 'RESERVED') {
    return {
      text: 'Der Artikel ist aktuell reserviert. Neue Abschluesse sind vorerst blockiert.',
      actionHref: '/offers',
      actionLabel: 'Zu Angeboten',
    };
  }
  ...
  if (offer.product.status === 'HIDDEN') {
    return {
      text: 'Das Listing ist nicht mehr oeffentlich verfuegbar.',
      actionHref: '/offers',
      actionLabel: 'Zu Angeboten',
    };
  }

  if (offer.status === 'ACCEPTED' && currentUserId === offer.buyerId) {
    return {
  ...
  if (offer.status === 'ACCEPTED') {
    return {
      text: 'Das Angebot wurde angenommen. Jetzt kann der Kaeufer den Checkout abschliessen.',
      actionHref: '/offers',
      actionLabel: 'Zu Angeboten',
    };
  }
  ...
  if (offer.status === 'PENDING') {
    return {
      text: 'Diese Verhandlung ist noch offen.',
      actionHref: '/offers',
      actionLabel: 'Verhandlung oeffnen',
    };
  }

  if (offer.status === 'COUNTERED') {
    return {
      text: 'Dieser Stand wurde von einer neueren Verhandlungsrunde abgeloest.',
      actionHref: '/offers',
      actionLabel: 'Zu Angeboten',
    };
  }
  ...
  if (offer.status === 'REJECTED') {
    return {
      text: 'Diese Verhandlungsrunde wurde beendet.',
      actionHref: '/offers',
      actionLabel: 'Zu Angeboten',
    };
  }
  ...
  return {
    text: 'Der aktuelle Status ist in deiner Angebotsuebersicht verfuegbar.',
    actionHref: '/offers',
    actionLabel: 'Zu Angeboten',
  };
}
...
export default function InboxPage() {
  ...
  const activeOfferSummary = activeOffer ? getOfferSummary(activeOffer, session?.user?.id) : null;
    ...
    const loadConversations = async () => {
      try {
        ...
        if (result.success) {
          ...
          if (!requestedPartnerId && !activePartnerId && result.data?.length) {
  ...
  }, [activePartnerId, requestedPartnerId]);
  ...
  useEffect(() => {
    const loadConversation = async () => {
      ...
      try {
        const response = await fetch(`/api/messages/${activePartnerId}`);
        const result = await response.json();
        if (result.success) {
  ...
  useEffect(() => {
    ...
    const handleNewMessage = async (event: RealtimeMessageEvent) => {
      ...
      const isIncoming = event.message.receiverId === userId;
      ...
      setConversations((current) => {
        ...
        return sortConversations(
          current.map((conversation) =>
            conversation.partnerId === event.partnerId ? nextConversation : conversation
          )
        );
      });
      ...
      if (isActiveThread) {
        ...
        if (isIncoming) {
          try {
            await fetch(`/api/messages/${event.partnerId}/read`, { method: 'POST' });
          } catch {
            ...
            current.map((conversation) =>
              conversation.partnerId === activePartnerId
                ? {
                    ...
                    offer: result.data.offer || conversation.offer || null,
                  }
                : conversation
            )
          );
          ...
          <div className="flex-1 overflow-y-auto">
            {loadingConversations ? (
              ...
              conversations.map((conversation) => {
                ...
                const conversationOfferSummary = conversation.offer
                  ? getOfferSummary(conversation.offer, session?.user?.id)
                  : null;
                return (
                  <button
              ...
              <div className="p-6 text-sm" style={{ color: 'var(--color-text-secondary)' }}>
                Noch keine Unterhaltungen. Starte von einem Produkt aus die erste Nachricht.
              </div>
            )}
          </div>
        </div>
            ...
            {!activePartnerId ? (
              ...
              <>
                ...
                {messages.length > 0 ? messages.map((message) => {
                  ...
                  const messageOfferSummary = message.offer ? getOfferSummary(message.offer, session?.user?.id) : null;
                  return (
